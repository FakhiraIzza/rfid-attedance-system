...
$subtitle='Lihat data presensi siswa pada kelas yang diampu';
...
// Auto mark Alfa after 12:00 if no attendance recorded today (per class)
...
if ($nowTime >= '12:00:00' && $weekday <= 5) {
  $autoStmt = $pdo->prepare("
    ...
    WHERE s.class_level = ?
      ...
      AND a.id_siswa IS NULL
  ");
  $autoStmt->execute([$todayDate, $todayDate, $todayDate, $classLevel]);
}

// Manual add or edit attendance (only for own class)
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['action'] ?? '')==='manual_add') {
  ...
  $redirectTo = trim((string)($_POST['redirect_to'] ?? '/guru/attendance.php'));
  if (!preg_match('#^/guru/[a-z0-9_./-]+\.php$#i', $redirectTo)) {
    $redirectTo = '/guru/attendance.php';
  }
  $evidenceFile = $_FILES['evidence_photo'] ?? null;
  $_SESSION['attendance_manual_form_guru'] = [
    'id_siswa' => $idSiswa,
    'tanggal' => $date,
  ...
  $cek = $pdo->prepare("SELECT COUNT(*) FROM students s WHERE s.id_siswa=? AND s.class_level=? AND $activeStudentWhere");
  $cek->execute([$idSiswa, $classLevel]);
  $okStudent = ((int)$cek->fetchColumn() > 0);

  if (!$okStudent || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $date) || !in_array($status, $validStatus, true)) {
    ...
    try {
      ...
      if (student_schema_has_column($pdo, 'attendance', 'class_level_snapshot')) {
        ...
        ");
        $stmt->execute([$idSiswa, $scanAt, $date, $type, $status, $classLevel, $catatan !== '' ? $catatan : null]);
      } else {
        $stmt = $pdo->prepare("
          INSERT INTO attendance (id_siswa, scan_at, tanggal, type, status, source, catatan)
          VALUES (?, ?, ?, ?, ?, 'manual', ?)
...
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['action'] ?? '')==='update_attendance') {
  ...
  if ($idAbsensi > 0 && in_array($status, $validStatus, true)) {
    ...
    $stmt = $pdo->prepare("
      ...
      SET a.status=?, a.type=?, a.catatan=?
      WHERE a.id_absensi=? AND s.class_level=?
    ");
    $stmt->execute([$status, $type, $catatan !== '' ? $catatan : null, $idAbsensi, $classLevel]);
    $_SESSION['flash_success'] = 'Absensi diperbarui.';
  } else {
    $_SESSION['flash_error'] = 'Data absensi tidak valid.';
  }

  header('Location: ' . base_url('/guru/attendance.php'));
  exit;
}
  ...
  if ($idAbsensi > 0) {
    try {
      ...
      $stmt = $pdo->prepare("
        ...
        WHERE a.id_absensi=? AND s.class_level=?
      ");
      $stmt->execute([$idAbsensi, $classLevel]);
      $pdo->commit();
...
$stmt = $pdo->prepare("
  SELECT a.id_absensi, a.tanggal, a.scan_at, a.type, a.status, a.catatan, s.nama_siswa,
...
$students = $pdo->prepare("SELECT s.id_siswa, s.nis, s.nama_siswa FROM students s WHERE s.class_level=? AND $activeStudentWhere ORDER BY s.nama_siswa");
$students->execute([$classLevel]);
$kids = $students->fetchAll();

$manualForm = $_SESSION['attendance_manual_form_guru'] ?? [];
...
if ($manualStudentId > 0) {
  foreach ($kids as $k) {
    if ((int)$k['id_siswa'] === $manualStudentId) {
      $manualStudentLabel = $k['nis']." | ".$k['nama_siswa'];
      break;
    }
  }
}
...
if ($studentId > 0) {
  foreach ($kids as $k) {
    if ((int)$k['id_siswa'] === $studentId) {
      $studentLabel = $k['nis']." | ".$k['nama_siswa'];
      break;
    }
  }
}
...
");
$missingMasukStmt->execute([$today, $classLevel]);
...
$missingPulangStmt = $pdo->prepare("
  ...
  WHERE s.class_level = ?
    AND $activeStudentWhere
    AND ap.id_siswa IS NULL
  ORDER BY s.nama_siswa
");
$missingPulangStmt->execute([$today, $today, $classLevel]);
...
<div class="container-fluid">
  <div class="row">
    ...
    <div class="col-12 col-lg-9 col-xl-10 p-4">
        ...
        <div class="card-body">
          ...
          <form class="row g-2 align-items-end mt-2" method="get">
            <div class="col-12 col-md-4">
              ...
              <input id="studentSearchFilterGuru" list="studentListFilterGuru" class="form-control" placeholder="Ketik nama/NIS..." autocomplete="off" value="<?= htmlspecialchars($studentLabel) ?>">
              <datalist id="studentListFilterGuru">
                <?php foreach($kids as $k): ?>
                  <?php $label = $k['nis']." | ".$k['nama_siswa']; ?>
                  <option value="<?= htmlspecialchars($label) ?>" data-id="<?= (int)$k['id_siswa'] ?>"></option>
                <?php endforeach; ?>
              </datalist>
              <input type="hidden" name="student_id" id="studentIdFilterGuru" value="<?= $studentId > 0 ? (int)$studentId : '' ?>">
            </div>
            ...
            <div class="col-12 col-md-2">
              <a class="btn btn-outline-primary w-100" href="<?= base_url('/guru/export_attendance.php?from=' . urlencode($from) . '&to=' . urlencode($to) . '&type_filter=' . urlencode($typeFilter) . '&student_id=' . urlencode((string)$studentId) . '&layout=detail') ?>">CSV Detail</a>
            </div>
            <div class="col-12 col-md-2">
              <a class="btn btn-outline-secondary w-100" href="<?= base_url('/guru/attendance_print.php?from=' . urlencode($from) . '&to=' . urlencode($to) . '&type_filter=' . urlencode($typeFilter) . '&student_id=' . urlencode((string)$studentId) . '&layout=detail') ?>">PDF Detail</a>
            </div>
            <div class="col-12 col-md-2">
              <a class="btn btn-outline-success w-100" href="<?= base_url('/guru/export_attendance.php?from=' . urlencode($from) . '&to=' . urlencode($to) . '&type_filter=' . urlencode($typeFilter) . '&student_id=' . urlencode((string)$studentId) . '&layout=book') ?>">CSV Buku Absen</a>
            </div>
            <div class="col-12 col-md-2">
              <a class="btn btn-outline-dark w-100" href="<?= base_url('/guru/attendance_print.php?from=' . urlencode($from) . '&to=' . urlencode($to) . '&type_filter=' . urlencode($typeFilter) . '&student_id=' . urlencode((string)$studentId) . '&layout=book') ?>">PDF Buku Absen</a>
            </div>
          </form>
      ...
      <div class="card shadow-sm mb-3">
        <div class="card-body">
          <div class="fw-semibold mb-2">Tambah Absensi Manual (Kelas <?= htmlspecialchars($classLevel) ?>)</div>
          <form method="post" class="row g-2 align-items-end" enctype="multipart/form-data">
            <input type="hidden" name="action" value="manual_add">
            <input type="hidden" name="redirect_to" value="/guru/attendance.php">
            <div class="col-12 col-md-4">
              <label class="form-label">Siswa</label>
              <input id="studentSearchGuru" list="studentListGuru" class="form-control" placeholder="Ketik nama/NIS..." autocomplete="off" value="<?= htmlspecialchars($manualStudentLabel) ?>" required>
              <datalist id="studentListGuru">
                <?php foreach($kids as $k): ?>
                  <?php $label = $k['nis']." | ".$k['nama_siswa']; ?>
                  <option value="<?= htmlspecialchars($label) ?>" data-id="<?= (int)$k['id_siswa'] ?>"></option>
                <?php endforeach; ?>
              </datalist>
              <input type="hidden" name="id_siswa" id="studentIdGuru" value="<?= $manualStudentId > 0 ? (int)$manualStudentId : '' ?>">
            </div>
              ...
              <div class="table-responsive">
                <table class="table table-sm align-middle">
                  ...
                  <tbody>
                    ...
                    <?php endforeach; ?>
                    <?php if (!$missingMasuk): ?>
                      <tr><td class="small-muted">Semua siswa sudah absen masuk.</td></tr>
                    <?php endif; ?>
                  </tbody>
                </table>
              </div>
                ...
                <table class="table table-sm align-middle">
                  ...
                  <tbody>
                    ...
                    <?php if (!$missingPulang): ?>
                      <tr><td class="small-muted">Semua siswa sudah absen pulang.</td></tr>
                    <?php endif; ?>
                  </tbody>
                </table>
              </div>
      ...
      <div class="card shadow-sm">
        <div class="card-body">
          ...
          <div class="table-responsive">
            <table class="table table-sm align-middle">
              <thead><tr><th>Tanggal</th><th>Waktu</th><th>Nama</th><th>Status</th><th>Keterangan</th><th>Evidence</th><th>Aksi</th></tr></thead>
              <tbody id="attendanceBody">
                <?php foreach($rows as $r): ?>
                  <?php
                    ...
                    $searchText = strtolower(trim(($r['nama_siswa'] ?? '') . ' ' . $statusLabel . ' ' . ($r['tanggal'] ?? '') . ' ' . $jenis));
                  ?>
                ...
                <?php endforeach; ?>
                <?php if(!$rows): ?>
                  <tr id="attendanceEmpty"><td colspan="7" class="small-muted">Tidak ada data pada rentang ini.</td></tr>
                <?php else: ?>
                  <tr id="attendanceEmpty" style="display:none"><td colspan="7" class="small-muted">Tidak ada data pada rentang ini.</td></tr>
                <?php endif; ?>
              </tbody>
            </table>
          </div>
...
const studentSearchGuru = document.getElementById("studentSearchGuru");
const studentIdGuru = document.getElementById("studentIdGuru");
const studentListGuru = document.getElementById("studentListGuru");
const studentSearchFilterGuru = document.getElementById("studentSearchFilterGuru");
const studentIdFilterGuru = document.getElementById("studentIdFilterGuru");
const studentListFilterGuru = document.getElementById("studentListFilterGuru");
...
studentSearchGuru?.addEventListener("input", () => {
  resolveStudentId(studentSearchGuru, studentListGuru, studentIdGuru);
});
studentSearchGuru?.addEventListener("blur", () => {
  resolveStudentId(studentSearchGuru, studentListGuru, studentIdGuru);
});
studentSearchFilterGuru?.addEventListener("input", () => {
  resolveStudentId(studentSearchFilterGuru, studentListFilterGuru, studentIdFilterGuru);
});
studentSearchFilterGuru?.addEventListener("blur", () => {
  resolveStudentId(studentSearchFilterGuru, studentListFilterGuru, studentIdFilterGuru);
});
...
document.addEventListener('click', (e) => {
  ...
  if (btn.getAttribute('data-action') === 'edit-attendance') {
    openEditModal({
